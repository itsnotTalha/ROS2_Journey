...
class ControllerMenu:
    ...
    def _topic_active(self, topic_name: str) -> bool:
        """Check ROS2 graph to see if a topic currently exists."""
        if not ROS2_AVAILABLE or not self.ros_initialized or not self.teleop_node:
            return False
        try:
            topics = self.teleop_node.get_topic_names_and_types()
    ...
    def draw(self, stdscr):
        ...
        # Drive ROS status based on command topic /buswala
        d_ros_active = self._topic_active("/buswala")
        ...
        # ARM ROS status based on command topic /aram
        a_ros_active = self._topic_active("/aram")
